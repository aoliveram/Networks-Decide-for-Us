## ==== Regressions (final): Adjusted for current project structure, robust load, BAM ==========
...
# Adjusted Output Directories
OUT_DIR_MODELS   <- "output/06_regression_analysis/models"
OUT_DIR_FIGS     <- "plots/06_regression_analysis"
dir.create(OUT_DIR_MODELS, showWarnings = FALSE, recursive = TRUE)
dir.create(OUT_DIR_FIGS,   showWarnings = FALSE, recursive = TRUE)

# --------------------------- 1) Directorios --------------------------------
# Adjusted Input Directories
dir_atp_emp <- "output/04_ATP_diffusion_sims"
...
ls_grand_combined <- function(path) {
  ...
  # Pattern matches: results_m03-06_all_STRATEGY.rds
  f <- list.files(path, pattern = "results_m.*_all_.*\\.rds$", full.names = TRUE, recursive = FALSE)
  f
}
...
parse_key_into_cols <- function(df, key){
  # Used if the RDS contains a nested list structure with keys like "sd_0.08"
...
parse_seed_from_filename <- function(path) {
  b <- basename(path)
  # Matches results_m03-06_all_STRATEGY.rds
  # Strategy is at the end before .rds
  s <- str_match(b, "_([a-zA-Z]+)\\.rds$")[,2]
  seed <- s
...
if (all(c("num_adopters","N_nodes_actual") %in% names(df_all))) {
  message("\n--- Running Count-based Binomial Models ---")
  ...
  plot_surface(m_bam_total_counts, "bam_total_counts")
  
  # Social (si hay conteos y columna)
  if("num_adopted_social" %in% names(df_all)) {
    ...
    plot_surface(m_bam_social_counts, "bam_social_counts")
  }
  
  # Racional (si hay conteos)
  if("num_adopted_rational" %in% names(df_all)) {
    ...
    plot_surface(m_bam_rational_counts, "bam_rational_counts")
  }
