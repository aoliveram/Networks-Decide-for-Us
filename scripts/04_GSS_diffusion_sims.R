...
# Generates (rds):
#   output/04_GSS_diffusion_sims/phase_transition_GRAND_COMBINED_raw_results_all_sds_means_XXX.
# where XXX == #"closeness" #"marginal" #"eigen" #"central" #"random"
# No plots are generated in this script.
# Sources:
# - 'GSS' Networks: data/02_GSS_network_ergm/GSS_net_sim_1000_XXX.rds
# - Simulation functions: netdiffuseR (stochastic-transmission branch)
#
# # Core Parameters
# NUM_SEED_RUNS_TOTAL     : 24 (runs per mean and deviation combination)
...
run_diffusion_simulation <- function(
    SEEDING_STRATEGY_FIXED,
    THRESHOLD_MEAN_SWEEP_LIST,
    TAU_NORMAL_SD_SWEEP_LIST,
    CURRENT_GRAPH_TYPE_LABEL,
    IUL_VALUES_SWEEP,
    H_VALUES_SWEEP,
    NUM_SEED_RUNS_TOTAL = 24, # Adapted default, normally 96
    NUM_CORES_TO_USE = 8,
  ...
  cat(paste0("\n********************************************************************\n"))
  cat(paste0("STARTING SIMULATION FOR STRATEGY: ", SEEDING_STRATEGY_FIXED, "\n"))
  cat(paste0("********************************************************************\n"))
  ...
  # Define timing_file early to avoid scope issues if loop is skipped
  ...
  for (sd_idx in 1:total_sd_iterations) {
    ...
    for (mean_idx in 1:total_mean_iterations) {
      ...
      list_of_results_for_this_mean_sd_combo <- foreach(
        ...
        current_network_path <- paste0(NETWORKS_DIR, "GSS_net_sim_1000_", sprintf("%03d", network_file_idx), ".rds")
    ...
    saveRDS(current_sd_all_means_raw_results, raw_data_filename_current_sd)
    ...
    # timing_file is already defined at the top
  ...
  stopCluster(cl)
  cat("\nAll simulation sweeps complete for strategy: ", SEEDING_STRATEGY_FIXED, "\n")

  # -----------------------------------------------------------------------------
  # Final Grand Data Saving
  # -----------------------------------------------------------------------------
