...
dir_input <- "data/02_GSS_network_ergm"
pattern_rds <- "^GSS_network_simulated_1000_\\d{3}\\.rds$"
...
#   En nuestras redes GSS, sigma ≈ 2.1 sugiere small-world robusto (alto C sin penalizar demasiado L).
...
# Comparación de grados (GSS vs ER/WS/BA/CONFIG)
degree_cmp_tbl <- map_dfr(res_list, ~ .x$degree_cmp %>% mutate(id = .x$id))

# CCDF log-log “condensada” GSS vs BA
...
# Para GSS: condensado sobre 100 redes (media y sd de p_k por k).
...
## Condensar GSS: media y sd por k (promediando sobre las 100 redes)
## Nota: completamos los grados faltantes por red con cero para no sesgar la media.
deg_gss_df <- map_dfr(res_list, ~{
  tibble(k = as.integer(degree(.x$g)))
}, .id = "id")

k_max_gss <- max(deg_gss_df$k, na.rm = TRUE)

# Conteo por red y grado
deg_gss_counts <- deg_gss_df %>%
  group_by(id, k) %>%
  summarise(n = n(), .groups = "drop")

# Completar grados faltantes 0..k_max_gss con n=0 por red
deg_gss_counts_full <- deg_gss_counts %>%
  group_by(id) %>%
  tidyr::complete(k = 0:k_max_gss, fill = list(n = 0L)) %>%
  ungroup()

# pk por red
deg_gss_pk <- deg_gss_counts_full %>%
  group_by(id) %>%
  mutate(pk = n / sum(n)) %>%
  ungroup()

# Media y desvío estándar por k (sobre redes)
deg_gss_pk_stats <- deg_gss_pk %>%
  group_by(k) %>%
...
p_lin <- ggplot() +
  ...
  geom_line(data = deg_gss_pk_stats, aes(x = k, y = mean_pk, color = "GSS (media)"), linewidth = 1, show.legend = FALSE) +
  geom_ribbon(data = deg_gss_pk_stats,
              aes(x = k, ymin = pmax(mean_pk - sd_pk, 0), ymax = mean_pk + sd_pk, fill = "GSS (±1 sd)"),
  ...
  scale_color_manual(values = c("BA (teórico)" = col_BA,
                                "ER (teórico)" = col_ER,
                                "GSS (media)"  = col_GSS)) +
  scale_fill_manual(values  = c("GSS (±1 sd)" = col_GSS)) +
...
gss_log <- deg_gss_pk_stats %>% filter(k >= 1, mean_pk > 0)
...
p_log <- ggplot() +
  ...
  geom_line(data = gss_log, aes(x = k, y = mean_pk, color = "GSS (media)"), linewidth = 1) +
  geom_ribbon(data = gss_log,
              aes(x = k, ymin = pmax(mean_pk - sd_pk, 0), ymax = mean_pk + sd_pk, fill = "GSS (±1 sd)"),
...
(p_lin | p_log) #+ plot_annotation(title = "Distribución de grado: GSS (rojo) vs ER (verde) y BA (morado; teóricos)")
ggsave("plots/02_GSS_network_ergm_tests/degree_dist.pdf", width = 11, height = 4.8, dpi = 300)
...
ggplot(sw_scaling, aes(x = logN, y = L, group = id)) +
  ...
  theme_minimal()
# ggsave("plots/02_GSS_network_ergm_tests/smallworld_subsampling.pdf", width = 7, height = 4.5, dpi = 300)
...
ggplot(filter(p1, is.finite(L) & is.finite(L_rw))) +
  ...
  theme_minimal()
# ggsave("plots/02_GSS_network_ergm_tests/rewiring_L.pdf", width = 5, height = 4, dpi = 300)
...
ggplot(p1) +
  ...
  theme_minimal()
# ggsave("plots/02_GSS_network_ergm_tests/rewiring_C.pdf", width = 5, height = 4, dpi = 300)

# ------------------------------------------------------------------------------
# 2) GSS vs BA
# ------------------------------------------------------------------------------

# C) CCDF log-log de grados: GSS (condensado) vs BA
...
ccdf_plot_df <- bind_rows(ccdf_gss, ba_tbl)

ggplot(ccdf_plot_df, aes(x = k, y = CCDF, color = model)) +
  geom_point(data = subset(ccdf_plot_df, model == "GSS (100 redes)"), alpha = 0.6) +
  geom_line(data = subset(ccdf_plot_df, model == "Barabási–Albert"), linewidth = 1) +
  scale_x_log10() + scale_y_log10() +
  labs(title = "CCDF log-log: GSS (condensada) vs BA",
       x = "k (log)", y = "P(K≥k) (log)", color = NULL) +
  theme_minimal()
# ggsave("plots/02_GSS_network_ergm_tests/degree_ccdf_loglog_GSS_vs_BA.pdf", width = 6.5, height = 4.5, dpi = 300)
...
ggplot(triad_long, aes(x = triad, y = prop)) +
  ...
  labs(title = "Censo triádico (GSS): p1, p2, p3 con referencia ER (línea roja)",
       x = "Tipo de triada", y = "Proporción") +
  theme_minimal()
# Referencia ER: p_bar = mean(density); p1=3p(1-p)^2, p2=3p^2(1-p), p3=p^3.
# ggsave("plots/02_GSS_network_ergm_tests/triadic_census_summary.pdf", width = 7, height = 4, dpi = 300)
...
ggplot(Ck_summary, aes(x = k, y = Ck_med)) +
  ...
  labs(title = "C(k) vs k (GSS, mediana y bandas [p25,p75])",
       x = "k", y = "C(k)") + theme_minimal()
ggsave("plots/02_GSS_network_ergm_tests/Ck_vs_k.pdf", width = 6.5, height = 4, dpi = 300)
...
ggplot(percolation_summary, aes(x = p, y = S_med, color = mode)) +
  ...
  theme_minimal()
# ggsave("plots/02_GSS_network_ergm_tests/percolation_curves.pdf", width = 6.5, height = 4, dpi = 300)
...
# Ratios triádicos vs ER (usar medianas GSS)
r1 <- p1_med / p1_ER
r2 <- p2_med / p2_ER
r3 <- p3_med / p3_ER
...
writeLines(tex_lines, con = "output/02_GSS_network_ergm_tests/tabla_resumen_GSS.tex")

message('Archivo LaTeX escrito en: output/02_GSS_network_ergm_tests/tabla_resumen_GSS.tex. Compila con: pdflatex output/02_GSS_network_ergm_tests/tabla_resumen_GSS.tex')
