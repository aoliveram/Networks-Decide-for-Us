...
\centering
Conditional on \emph{fixed individual characteristics},
...
\begin{tikzpicture}[every node/.style={font=\footnotesize}]
  \node[draw,rounded corners,fill=ndreddark!8,text width=2.7cm,align=center,
        minimum height=1.25cm] (a) {Homophily strengths $\hat{\boldsymbol{\beta}}$ \refn{3,4}};
  \node[draw,rounded corners,fill=ndreddark!8,text width=2.9cm,align=center,
        minimum height=1.25cm,right=1.0cm of a] (b) {Construct plausible networks};
  \node[draw,rounded corners,fill=ndreddark!8,text width=2.9cm,align=center,
...
\end{tikzpicture}\\[-1pt]
{\scriptsize \textbf{Path 1 — vary $H$ at fixed $T$:} exponent $\delta$.}
\column{0.48\textwidth}
\footnotesize
\textbf{\color{ndgreen}Path 2 — fix IUL, vary MSP ($T$)}
\begin{itemize}\footnotesize
  \item the temperature route to the critical point
  \item if continuous, $\chi=\mathrm{Var}(\Phi)\sim|MSP-MSP_c|^{-\gamma}$
\end{itemize}
\vspace{0.2cm}
\textbf{\color{ndblue}Path 1 — fix MSP, vary IUL ($H$)}
\begin{itemize}\footnotesize
...
{\footnotesize \textbf{Top:} Total, Rational and Social adoption. Rational adoption grows smoothly with
IUL.

\textbf{Bottom-left:} Susceptibility $\chi=\mathrm{Var}(\Phi)$; \textbf{Bottom-right:} relaxation-time SD
divergence.}
\end{frame}
...
\column{0.5\textwidth}
\centering
\includegraphics[width=0.99\linewidth]{../../playground/checks-claude/plots/camino_H_critical_isotherm.pdf}\\
{\footnotesize Critical isotherms $\Phi(\mathrm{IUL})$ at $MSP=MSP_c$.}
\column{0.5\textwidth}
...
\begin{itemize}\small
  ...
  \item The field (IUL) direction does \emphr{not} distinguish the two topologies.
\end{itemize}
\end{columns}
\end{frame}

% ---- Result 3 (part 2) = UNIVERSALITY (point 8 reframe ; point 15 fixed table)
\begin{frame}{Result 3 — Universality exclusive to realistic networks}
\begin{columns}[c]
\column{0.5\textwidth}
\centering
\includegraphics[width=0.95\linewidth]{../../plots/07_phase_transition/GSS/method4_exponents.pdf}\\
{\footnotesize $\chi\sim|MSP-MSP_c|^{-1}$.}
\column{0.5\textwidth}
\renewcommand{\arraystretch}{1.25}
{\footnotesize
\begin{tabular}{@{}lcc@{}}
...
\vspace{0.2cm}
\begin{center}\small
\emph{Macroscopic social change is not the sum of individual preferences 

— it's governed by the structure of our real topology}.
\end{center}
\end{frame}
...
\begin{frame}[plain]
  ...
  \begin{center}
    ...
    \qrcode[height=2.4cm]{https://doi.org/10.5281/zenodo.19360209}\\[0.3cm]
    {\small Scan to see the preprint draft at Zenodo!}\\[5pt]
    {\texttt{an.oliveram@udd.cl} $\;\cdot\;$ \texttt{aoliveram.cl}}\\[7pt]
    \vspace{0.3cm}
