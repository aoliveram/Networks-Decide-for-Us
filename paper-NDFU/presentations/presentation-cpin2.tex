...
\begin{itemize}
  ...
  \item Individual \emph{preferences} to adopting behaviors can be measured in surveys.
\end{itemize}
\begin{center}
...
\begin{itemize}
  \item The attractiveness of the innovation is its
        \emph{Intrinsic Utility Level} \emphr{IUL}, $\Gamma\in[0,1]$.
...
% ================================================================ SETTING UP
\sectionslide{Constructing networks \& running the model}
...
\vspace{0.3cm}
\begin{center}\small
Does adoption have a critical point? — and if so, what kind of transition is it?
\end{center}
\end{frame}
...
\column{0.52\textwidth}
\centering
% --- Plot B: Path 1 — vary H (IUL) at fixed T  (now on TOP) ---
...
\end{tikzpicture}\\[-1pt]
{\scriptsize \textbf{Path 1 — fix $T$, vary $H$:} exponent $\delta$.}\\[7pt]
% --- Plot A: Path 2 — vary T (MSP) at fixed IUL  (now on BOTTOM) ---
\begin{tikzpicture}[scale=0.62, font=\tiny, >={Latex[length=1.6mm]}]
...
\end{tikzpicture}\\[-1pt]
{\scriptsize \textbf{Path 2 — fix $H$, vary $T$:} exponent $\gamma$.}
\column{0.48\textwidth}
\footnotesize
...
\begin{itemize}\footnotesize
  ...
  \item Randomization $\Rightarrow$ \emphr{$\gamma\approx4$}. Rewire the network and the mean-field behavior is lost.
  \item Universality means the microscopic details don't matter.
\end{itemize}
\end{columns}
\end{frame}
...
\vspace{0.10cm}
\begin{itemize}\small
  \item \emphr{Dynamics:} on a realistic network the \emph{aggregate} pattern is \emphr{continuous}.
  \item Aggregate phenomenon follows the
        familiar \emphr{Rogers S-curve}, across very different diffusions as fax machines, hashtags,
        and protest waves.
\end{itemize}
\vspace{0.2cm}
\begin{center}\small
