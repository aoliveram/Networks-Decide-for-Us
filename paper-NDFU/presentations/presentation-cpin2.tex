...
If intrinsic adoption fails, agents may adopt by peer contagion. Effective exposure:
\begin{equation*}
...
\begin{itemize}
  ...
  \item \emphr{MSP} (Maximum Social Proximity): how far across demographic lines influence can reach.
\end{itemize}
...
\begin{frame}{Imputing a network onto a survey \refn{2,3}}
\begin{columns}[T]
...
\end{itemize}
\vspace{0.2cm}
Homophily strengths $\hat{\boldsymbol\beta}$ from GSS confidant data \refn{4} feed an \emphr{ERGM}:
...
Hybrid social contagion via \texttt{netdiffuseR}, over plausible (GSS) vs.\
...
Susceptibility diverges near criticality: $\;\chi=\mathrm{Var}(\Phi)\propto |MSP-MSP_c|^{-\gamma}$
\end{center}
\end{frame}
