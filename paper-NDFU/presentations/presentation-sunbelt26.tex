...
{\footnotesize \emphr{Homophily is destroyed}: every Blau-axis assortativity collapses to
$\approx 0$, and the social distance $d_{ij}$ rises \emphr{+42\%}.}
\end{frame}
...
\includegraphics[trim={0 4.95cm 0 0.9cm}, clip, width=0.98\textwidth]{../abstract-sn26/figure1_5panels_GSS.pdf}\\[4pt]
{\footnotesize Mean adoption over the attractiveness (IUL) -- openness (MSP) plane.
\emphr{Rational} adoption grows smoothly with attractiveness; \emphr{social} adoption is more
complex, so total adoption stays high over a wide region — \emph{even when the behavior is
barely attractive}, social influence alone carries it.}
\end{frame}

% ---- 7b. RESULT 2: structural premium ----
\begin{frame}{Result 2 — A structural premium}
\begin{columns}[c]
...
\begin{tabular}{@{}lcc@{}}
\toprule
Critical exponents & \textbf{GSS} & \textbf{GSS-DP} \\
\midrule
Suscept. $\gamma$ & $\approx 0.91$ & $\approx 3.9$ \\
Isotherm $\delta$ & $\approx 2.85$ & $\approx 2.91$ \\
\midrule
...
{\scriptsize Critical isotherms $\Phi(\mathrm{IUL})$ at $MSP{=}MSP_c$: \emph{both} topologies
give $\delta\!\approx\!3$. The field axis does \emph{not} discriminate — only $\gamma$ (the
temperature axis) does.}
\end{columns}
\end{frame}
