...
To formally capture the interaction probability, we construct a distance vector $\mathbf{d}_{ij}$ for each dyad. As adapted from \citet{smith2014social}, categorical differences operate as simple mismatch indicators $d^{\mathrm{cat}}_{ij} = \mathbb{I}(x^{\mathrm{cat}}_i \neq x^{\mathrm{cat}}_j)$, while continuous variables measure absolute distance $d^{\mathrm{cont}}_{ij} = |x^{\mathrm{cont}}_i - x^{\mathrm{cont}}_j|$. These components serve as predictors in a case-control logistic regression, establishing log-odds penalizations for socio-demographic disparities (Table \ref{tab:smith2014}).

\begin{table}[H]
    \centering
    \caption{Logistic Regression predicting confiding relationship probability based on social distance (Smith et al., 2014).}
    \label{tab:smith2014}
    \begin{tabular}{lcc}
    \toprule
    ...
    Year & $-0.179^{***}$ & $-0.052$ \\
    \bottomrule
    \multicolumn{3}{l}{\footnotesize $^{***} p < .001$. Standard errors and year-interactions omitted.}
    \end{tabular}
\end{table}
