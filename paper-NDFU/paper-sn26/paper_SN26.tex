...
To explore structural causalities, we conducted massive exhaustive parameter sweeps traversing millions of interactions. We swept the Intrinsic Utility Level (IUL) from 0.0 to 1.0 ($\Delta = 0.025$) and the system-wide threshold limit (MSP) continuously from 0 to 1 ($\Delta = 1/12$). Individual threshold parameters $\tau_i$ were seeded across normal distributions $\sim \mathcal{N}(\mu, \sigma)$ sweeping $\mu \in [0.3, 0.6]$ and $\sigma \in [0.08, 0.20]$. Each configuration was re-evaluated through over 24 runs varying algorithmic seeding rules (Random, Central, Marginal, Closeness, Eigenvector).
