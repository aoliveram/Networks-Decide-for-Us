...
\begin{table}[H]
    \centering
    \caption{Logistic Regression predicting confiding relationship probability based on social distance (Smith et al., 2014). Model 1 (Base), Model 2 (Temporal Interactors).}
    \label{tab:smith2014}
    \begin{tabular}{lcc}
    \toprule
    \textbf{Variable} & \textbf{Model 1 ($\beta$)} & \textbf{Model 2 ($\gamma,\dots$)} \\
    \midrule
    Intercept & $-14.456^{***}$ & $-14.519^{***}$ \\
    Different race ($d^{\mathrm{cat}}_{ij}$) & $-1.819^{***}$ & $-1.959^{***}$ \\
    Different religion ($d^{\mathrm{cat}}_{ij}$) & $-1.362^{***}$ & $-1.27^{***}$ \\
    ...
    Year & $-0.179^{***}$ & $-0.052$ \\
    \bottomrule
    \multicolumn{3}{p{0.9\linewidth}}{\footnotesize $^{***} p < .001$. Extracted homophilic strength used for topological imputation is $\hat{\boldsymbol{\beta}}_{\text{tot}} = \hat{\boldsymbol{\beta}} + \hat{\boldsymbol{\gamma}}$. Standard errors omitted.}
    \end{tabular}
\end{table}
